...
module.exports.listAccount = async (req, res) => {
  try {
    ...
    if (type === "manager")
      listAccount = await accountModel.find(
        { role: type },
        "-password -cart -role"
      );
    else
...
module.exports.updateAccount = async (req, res) => {
  try {
    ...
    let updateAccount = await accountModel.findByIdAndUpdate(
      id,
      { updateData },
      { new: true }
    );
    return res
      .status(200)
      .json({ message: "Update success", data: updateAccount });
  } catch (err) {
    return res.status(400).json({ message: err.message });
  }
};
...
module.exports.checkExist = async (req, res) => {
  try {
    ...
    let checkExist = await accountModel.findOne({ phone: phone });
    if (checkExist) flag = true;
    return res
      .status(200)
      .json({ message: "Check data success", data: { exist: flag } });
  } catch (err) {
    return res.status(400).json({ message: err.message });
  }
};
